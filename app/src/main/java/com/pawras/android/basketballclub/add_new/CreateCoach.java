...
import com.pawras.android.basketballclub.MainActivity;
...
public class CreateCoach extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.create_coach);
        initialization();
    }
            ...
            case R.id.create_coach:
                if (validate()) {
                    if (haveNetworkConnection()) {
                        loading = ProgressDialog.show(CreateCoach.this, "Creating Coach", "Please wait", false, false);
                        addCoachToFirebaseDatabase();
                    } else {
                        openDialog();
                    }
                }
    ...
    public void addCoachToFirebaseDatabase() {
        //perform auto increment
        SharedPreferences mSharedPreferences = getSharedPreferences("identity", MODE_PRIVATE);
        String id = mSharedPreferences.getString("c_id", "0");
        int idd = Integer.parseInt(id);
        idd++;
        SharedPreferences.Editor mEditor = mSharedPreferences.edit();
        mEditor.putString("c_id", Integer.toString(idd));
        mEditor.commit();

        //write to the database
        mFirebaseInstance = FirebaseDatabase.getInstance();
        mFirebaseDatabase = mFirebaseInstance.getReference("coaches");
        ...
        loading.dismiss();
        finish();
        Toast.makeText(this, "Coach Created", Toast.LENGTH_SHORT).show();
    }
